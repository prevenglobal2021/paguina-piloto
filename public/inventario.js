...
--------------------------------------------------------- */
function verEtiquetaQRInventario(itemId){
  const it = buscarItemInventario(itemId);
  if(!it) return;
  ...
  if(tbody){
    tbody.innerHTML = itemsFiltrados.map(it=>{
      ...
      return `<tr>
        ...
        <td>${fotosHtml||'—'}</td>
        <td>
          <button class="btn-custom btn-secondary-custom btn-sm-custom" onclick="verFichaQR(${it.id})" title="Ver ficha del ítem"><i class="fas fa-clipboard-list"></i></button>
          <button class="btn-custom btn-secondary-custom btn-sm-custom" onclick="verEtiquetaQRInventario(${it.id})" title="Imprimir etiqueta QR"><i class="fas fa-qrcode"></i></button>
        </td>
        <td>
