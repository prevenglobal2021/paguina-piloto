...
// Interpreta el mismo texto que produce el escáner de QR de la app normal
// (URL con ?equipo=<id>, un id numérico plano, o el texto de una etiqueta
// vieja por serie/qrId) — reutilizado aquí para que el SuperAdmin escanee
// exactamente las mismas etiquetas ya impresas, sin duplicar el formato.
function interpretarCodigoEquipo(texto) {
  try {
    const url = new URL(texto);
    const idParam = url.searchParams.get('equipo');
    if (idParam) return { porId: parseInt(idParam) || null };
  } catch (e) { /* no era una URL, se sigue con los otros métodos */ }
  if (/^\d+$/.test(texto.trim())) return { porId: parseInt(texto.trim()) };
  return { porTexto: texto.trim().toLowerCase() };
}

// Búsqueda de un equipo por su código QR (o su serie) EN TODAS LAS
// EMPRESAS a la vez — exclusiva de SuperAdmin, para hacerle seguimiento a
// cualquier equipo de cualquier cliente sin tener que entrar a cada
// empresa por separado. Los usuarios normales de cada empresa NUNCA tienen
// acceso a esta ruta (solo pasa por requireSuperAdmin).
app.get('/api/superadmin/equipos/buscar', requireSuperAdmin, async (req, res) => {
  const codigo = ((req.query.codigo || '') + '').trim();
  if (!codigo) return res.status(400).json({ error: 'Falta el código a buscar.' });
  const criterio = interpretarCodigoEquipo(codigo);

  const rEmpresas = await pool.query('SELECT slug, nombre, estado_app FROM empresas');
  for (const fila of rEmpresas.rows) {
    const clientes = (fila.estado_app && fila.estado_app.clientes) || [];
    for (const cliente of clientes) {
      const equiposConSede = (cliente.sedes || []).flatMap(s => (s.equipos || []).map(e => Object.assign({}, e, { sedeNombre: s.nombre })));
      const equiposSinSede = (cliente.equiposSinSede || []).map(e => Object.assign({}, e, { sedeNombre: null }));
      const todos = [...equiposConSede, ...equiposSinSede];
      const equipo = todos.find(e => {
        if (criterio.porId != null) return e.id === criterio.porId;
        return ((e.serie || '').toLowerCase() === criterio.porTexto) || ((e.qrId || '').toLowerCase() === criterio.porTexto);
      });
      if (equipo) {
        const ordenes = (fila.estado_app.ordenes || [])
          .filter(o => o.equipoId === equipo.id || (Array.isArray(o.equiposIds) && o.equiposIds.includes(equipo.id)))
          .map(o => {
            const esMultiEquipo = Array.isArray(o.equiposIds) && o.equiposIds.length > 1;
            const diagnostico = esMultiEquipo
              ? ((o.cierre && o.cierre.porEquipo && o.cierre.porEquipo[equipo.id]) || {}).diagnostico || null
              : (o.cierre ? o.cierre.diagnostico || null : null);
            return {
              numero: o.numero, fecha: o.fechaProgramada, estado: o.estado, tipo: o.tipo,
              prioridad: o.prioridad, diagnostico
            };
          })
          .sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''));
        return res.json({
          empresa: { slug: fila.slug, nombre: fila.nombre },
  ...
  res.status(404).json({ error: 'No se encontró ningún equipo con ese código en ninguna empresa registrada.' });
});
