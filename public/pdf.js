...
// GENERACIÓN DE PDF — punto único, reutilizado por Orden de Servicio,
// Cotización, Factura y Nómina. Captura DIRECTAMENTE el documento ya
// armado y visible dentro del modal — el mismo contenido que la persona
// ve en pantalla, con el mismo ancho fijo tipo hoja carta (definido en el
// CSS de .pdf-preview). Antes de capturar, se asegura de que el modal no
// esté desplazado por scroll de un documento anterior.
async function generarPDFDesdeElemento(idElementoOrigen, nombreArchivo){
  const elemento = document.getElementById(idElementoOrigen);
  const cajaModal = elemento.closest('.modal-box');
  if(cajaModal) cajaModal.scrollTop = 0;
